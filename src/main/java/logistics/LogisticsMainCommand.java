...
public class LogisticsMainCommand implements LogisticsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		EmployeeVO emvo = dao.information(idx);
...
//		일정
		...
		ArrayList<ScheduleVO> vos = sdao.getScMenu(mid, ym, 0);
		...
		ArrayList<BoardVO> bvos = bdao.getBoList(startIndexNo, pageSize);
		...
		ArrayList<NoticeVO> nvos = ndao.getNoList(startIndexNo, pageSize);
		request.setAttribute("nvos", nvos);
		//*****************************공지사항***********************
		
		//*****************************자료실***********************
		PdsDAO pdao = new PdsDAO();
		ArrayList<PdsVO> pvos = pdao.getPdsList(startIndexNo, pageSize);
		request.setAttribute("pvos", pvos);
		//*****************************자료실***********************
		
		request.setAttribute("vos", vos);
		request.setAttribute("emvo", emvo);
