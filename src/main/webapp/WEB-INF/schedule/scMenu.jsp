...
<head>
  ...
  <script>
    ...
    function scheduleInputView() {
    	...
    	scheduleInput += '<option value="회의">회의</option>';
    	scheduleInput += '<option value="결제">결제</option>';
    	scheduleInput += '<option value="기한">기한</option>';
    	scheduleInput += '<option value="회식">회식</option>';
    ...
    function updateCheck(idx, part, content) {
    	...
    	scheduleInput += '<span class="col"><input type="button" value="일정수정" onclick="scheduleUpdateOk('+idx+')" class="btn btn-change form-control"/></span>';
    	scheduleInput += '<span class="col"><input type="button" value="수정창닫기" onclick="scheduleUpdateClose('+idx+')" class="btn btn-secondary form-control"/></span>';
    	scheduleInput += '</span>';
    	scheduleInput += '</td></tr>';
    	scheduleInput += '</table>';
    	scheduleInput += '</form></div>';
...
<div class="container">
  ...
  <c:set var="scheduleCnt" value="${0}"/>
    <table class="table table-hover text-center">
      <tr class="th text-dark">
        <th>번호</th>
        <th>간단 내역</th>
        <th>분류</th>
        <th>비고</th>
      ...
      <c:forEach var="vo" items="${vos}" varStatus="st">
        <tr>
          ...
          <td>${vo.part}</td>
          <td>
            <input type="button" value="수정" onclick="updateCheck('${vo.idx}','${vo.part}','${fn:replace(vo.content,newLine,'<br/>')}')" id="scheduleUpdateOpen${vo.idx}" class="btn btn-change btn-sm"/>
