...
<div class="container">
  <div class="text-center">
    <button type="button" onclick="location.href='${ctp}/schedule.sc?yy=${yy-1}&mm=${mm}';" class="btn btn-primary btn-sm" title="이전년도">◁◁</button>
    <button type="button" onclick="location.href='${ctp}/schedule.sc?yy=${yy}&mm=${mm-1}';" class="btn btn-primary btn-sm" title="이전월">◀</button>
    <font size="5">${yy}년 ${mm+1}월</font>
    <button type="button" onclick="location.href='${ctp}/schedule.sc?yy=${yy}&mm=${mm+1}';" class="btn btn-primary btn-sm" title="다음월">▶</button>
    <button type="button" onclick="location.href='${ctp}/schedule.sc?yy=${yy+1}&mm=${mm}';" class="btn btn-primary btn-sm" title="다음년도">▷▷</button>
    &nbsp;&nbsp;
    <button type="button" onclick="location.href='${ctp}/schedule.sc';" class="btn btn-primary btn-sm" title="오늘날짜"><i class="fa fa-home"></i></button>
  </div>
  <br/>
  <div class="text-center">
    <table class="table table-bordered" style="height:450px">
      ...
      <tr>
        ...
        <c:forEach var="preDay" begin="${preLastDay-(startWeek-2)}" end="${preLastDay}">
          <td style="color:#777;font-size:0.6em">${prevMonth+1}-${preDay}</td>
        ...
        <c:if test="${nextStartWeek != 1}">
	        <c:forEach begin="${nextStartWeek}" end="7" varStatus="nextDay">
	          <td style="color:#777;font-size:0.6em">${nextMonth+1}-${nextDay.count}</td>
	        </c:forEach>
        </c:if>
      </tr>
    </table>
